...
const FeatureCard = ({ icon: Icon, title, description, index }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative p-10 rounded-[3rem] bg-slate-50 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/50 hover:border-emerald-500/50 transition-all duration-500 overflow-hidden"
    >
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-4 flex items-center gap-3">
          {title}
          <ArrowUpRight className="w-5 h-5 opacity-0 -translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all text-emerald-500" />
        </h3>
