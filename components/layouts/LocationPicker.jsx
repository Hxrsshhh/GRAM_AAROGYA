  ...
  useEffect(() => {
    if (mapInstance.current) {
      mapInstance.current.setStyle(
        `https://api.maptiler.com/maps/${viewMode}/style.json?key=${process.env.NEXT_PUBLIC_MAPTILER_KEY}`
      );
    }
  }, [viewMode]);
  ...
  const fetchNewAddress = async (lt, lg) => {
    ...
    try {
      const res = await fetch(
        `https://api.maptiler.com/geocoding/${lg},${lt}.json?key=${process.env.NEXT_PUBLIC_MAPTILER_KEY}`
      );
      const data = await res.json();
      onLocationChange(
        lt,
        lg,
        data.features?.[0]?.place_name || "Custom Point",
        true
      );
    } finally {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl h-64 group">
        ...
        <button
          onClick={() =>
            setViewMode(viewMode === "streets-v2" ? "hybrid" : "streets-v2")
          }
        ...
        >
          <div className="relative w-full h-full">
            <Image
              src={
                viewMode === "streets-v2"
                  ? "https://api.maptiler.com/maps/hybrid/static/0,0,1/80x80.png"
                  : "https://api.maptiler.com/maps/streets-v2/static/0,0,1/80x80.png"
              }
              alt="style"
              fill
        ...
        <div className="absolute bottom-3 right-3 z-10 flex flex-col bg-white dark:bg-slate-900 rounded-lg shadow-md border border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800">
          <button
            ...
            className="w-7 h-7 flex items-center justify-center text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
          >
            +
          </button>
          <button
