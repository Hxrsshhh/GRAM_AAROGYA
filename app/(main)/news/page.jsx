...
const translations = {
  English: {
    ...
    subheading: "Curated breakthroughs and health news from global verified sources.",
  ...
  Hindi: {
    ...
    subheading: "वैश्विक सत्यापित स्रोतों से नवीनतम चिकित्सा खोजें और स्वास्थ्य समाचार।",
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-gray-100 transition-colors duration-500 overflow-x-hidden">
      ...
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-full h-full -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-[600px] h-[600px] bg-blue-500/10 dark:bg-blue-600/10 blur-[120px] rounded-full animate-pulse" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[600px] h-[600px] bg-indigo-500/10 dark:bg-indigo-600/10 blur-[120px] rounded-full" />
      </div>
        ...
        <header className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 p-2 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[2rem] backdrop-blur-xl shadow-2xl shadow-blue-500/5">
            <div className="relative w-full sm:w-auto">
              ...
              >
                {languages.map((lang) => (
                  <option key={lang} value={lang} className="bg-white dark:bg-[#111]">
                    {lang}
                  </option>
                ))}
              </select>
        ...
        <main className="min-h-[500px]">
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="h-[400px] rounded-[2.5rem] bg-slate-200/50 dark:bg-white/5 animate-pulse border border-slate-200 dark:border-white/10" />
              ))}
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-slate-500 dark:text-gray-400 mb-8 max-w-xs mx-auto">{error}</p>
              <button onClick={handleGetNews} className="flex items-center gap-2 px-6 py-3 bg-slate-200 dark:bg-white/10 rounded-xl font-bold hover:bg-slate-300 transition-all">
        ...
        <div className="flex items-center justify-between mb-10 relative z-10">
          ...
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase">
              ...
              {new Date(article.date || Date.now()).toLocaleDateString(undefined, { 
                month: 'short', 
                day: 'numeric' 
              })}
            </div>
        ...
        <div className="relative z-10 flex-grow">
          ...
          <div className="relative">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-[15px] leading-relaxed font-medium line-clamp-4 group-hover:line-clamp-none transition-all duration-500">
              {article.description || article.content?.substring(0, 160) || "Comprehensive medical analysis and breakthrough data awaiting retrieval..."}
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-col items-center justify-center py-32 text-center space-y-8">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-500 dark:text-gray-400 max-w-xs mx-auto text-sm font-medium">
                  Select your language above to fetch the latest global medical intelligence.
                </p>
              </div>
            </div>
          )}
