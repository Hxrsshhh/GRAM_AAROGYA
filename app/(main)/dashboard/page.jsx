...
import {
  ...
  Clock,
  CheckCircle2,
  Activity, // Fixed import for Activity
} from "lucide-react";
import {
  AreaChart,
  Area,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white">
              System{" "}
              <span className="text-emerald-600 dark:text-emerald-400">Overview</span>
            </h1>
              ...
              <DashboardCard className="relative overflow-hidden group h-full">
                <div className="flex items-start justify-between relative z-10">
                  <div>
                    ...
                    <p className={`text-[11px] font-bold mt-2 ${stat.color} opacity-80`}>
                      {stat.change}
                    </p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
              ...
              <ResponsiveContainer width="99%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ backgroundColor: "rgba(15, 23, 42, 0.95)", borderRadius: "16px", border: "1px solid rgba(16, 185, 129, 0.2)", color: "#fff", fontSize: "12px", fontWeight: "800" }}
                  />
                  <Area type="monotone" dataKey="reports" stroke="#10b981" strokeWidth={4} fillOpacity={1} fill="url(#colorReports)" animationDuration={2000} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </DashboardCard>
          ...
          <DashboardCard>
            ...
            <div className="space-y-3">
              {[
                { label: "Report Anomaly", icon: Plus, path: "/issues/report", variant: "primary" },
                ...
                <Link href={action.path} key={i} className="block group">
                  <div className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all duration-300 ${action.variant === "primary" ? "bg-slate-900 dark:bg-emerald-600 text-white" : "bg-slate-50 dark:bg-slate-800/30 text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-800"} group-hover:translate-x-1 group-hover:shadow-lg group-hover:border-emerald-500/30`}>
            ...
            <div className="mt-8 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
              ...
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 leading-tight">All systems encrypted. Reports validated by the network.</p>
            </div>
          </DashboardCard>
        </div>
        ...
        <DashboardCard>
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/issues">
              <button className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 hover:text-emerald-500 transition-colors">View All Nodes</button>
            </Link>
          </div>
            ...
            {recentIssues.length > 0 ? (
              recentIssues.slice(0, 4).map((issue) => (
                <Link href={`/issues/${issue._id}`} key={issue._id}>
                  <motion.div whileHover={{ y: -4, scale: 1.01 }} className="cursor-pointer group relative flex items-center gap-4 p-4 rounded-[2rem] border border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all duration-300">
                    <div className="relative w-16 h-16 flex-shrink-0">
                      ...
                      />
                      <div className={`absolute -top-1 -right-1 px-2 py-0.5 text-[7px] font-black text-white rounded-full uppercase tracking-tighter ${issue.status === "resolved" ? "bg-emerald-500" : "bg-amber-500"}`}>
                        {issue.status}
                      </div>
                    </div>
