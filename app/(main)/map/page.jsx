    ...
    >
      ...
      <div className="absolute top-6 right-6 z-50">
        <button
          ...
          className="p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl hover:scale-105 transition-all"
        >
          {resolvedTheme === "dark" ? (
      ...
      <motion.div
        animate={{ width: isSidebarCollapsed ? "80px" : "384px" }}
        className="absolute top-6 left-6 bottom-6 z-50"
      >
          ...
          {!isSidebarCollapsed && (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loading ? (
                  <div className="text-center p-8 text-slate-400 animate-pulse font-bold text-xs">
                    Loading Civic Data...
                  </div>
                ) : (
                  filteredIssues.map((issue) => (
                    <div
                      ...
                      className={`group relative p-3 rounded-[1.5rem] cursor-pointer transition-all flex gap-4 items-center ${
                        selectedIssue?.id === issue.id
                          ? "bg-white dark:bg-slate-800/20 ring-1 ring-slate-500/20 scale-[1.02]"
                          : "bg-slate-50/80 dark:bg-slate-900/40 hover:bg-white dark:hover:bg-slate-800/20"
                      }`}
                    >
              ...
              <div className="p-4 space-y-3 bg-slate-50/30 dark:bg-slate-800/20 border-t border-slate-200 dark:border-slate-800">
                <Link href="/issues/report">
                  <button className="w-full p-4 mb-4 rounded-2xl border-2 border-dashed border-emerald-500/30 hover:border-emerald-500/60 transition-all text-emerald-600 font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
      ...
      <AnimatePresence mode="wait">
        {selectedIssue && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className={`absolute bottom-8 right-6 z-40 transition-all ${
              isSidebarCollapsed ? "left-28" : "left-[26.5rem]"
            }`}
          >
            <div className="bg-white/90 dark:bg-slate-900/95 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col md:flex-row max-w-3xl">
              ...
              <div className="flex-1 p-8 relative">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 rounded-full text-[11px] font-black uppercase">
