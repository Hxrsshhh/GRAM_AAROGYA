...
/**
 * UI COMPONENTS (CivicPulse Design System)
 */
const Badge = ({ children, variant = "pending" }) => {
  const styles = {
    pending: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200 dark:border-amber-800",
    "in-progress": "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800",
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${styles[variant] || styles.default}`}>
      {children}
    </span>
  );
};
...
const Button = ({ children, variant = "primary", onClick, leftIcon, className = "" }) => {
  const variants = {
    primary: "bg-emerald-600 text-white hover:bg-emerald-700 shadow-xl shadow-emerald-600/20",
    outline: "bg-transparent border-2 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-emerald-500 hover:text-emerald-500",
    ghost: "text-slate-500 hover:text-emerald-500"
  };

  return (
    <button 
      onClick={onClick}
      className={`flex items-center justify-center gap-2 px-8 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] transition-all active:scale-95 ${variants[variant]} ${className}`}
    >
      {leftIcon}
      {children}
    </button>
  ...
  useEffect(() => {
    const fetchIssue = async () => {
      try {
        ...
        setError("Unable to retrieve report details. Please try again later.");
        setLoading(false);
      }
    };
    fetchIssue();
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Syncing Civic Data...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <Card className="text-center max-w-md border-rose-500/20">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tighter mb-2">Report Not Found</h2>
          <p className="text-slate-500 text-sm font-medium mb-8 leading-relaxed">{error || "The requested issue ID does not exist in our public database."}</p>
        ...
        <div className="mb-8 mt-6">
          <button className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col items-start">
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mb-0.5">Return to</span>
              <span className="text-[11px] font-black uppercase tracking-widest text-slate-900 dark:text-white group-hover:text-emerald-500 transition-colors">Incident Feed</span>
            </div>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            <header className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-8 py-6 border-y border-slate-200 dark:border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{issue.viewCount} Engagements</span>
                </div>
                <div className="flex items-center gap-2">
                  <ThumbsUp className="w-4 h-4 text-emerald-500" />
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{issue.upvotes} Citizens Supported</span>
                </div>
              </div>
            </header>
            ...
            <section className="space-y-4">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500">Official Complaint</h3>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-8 border-t border-slate-200 dark:border-slate-800">
              ...
              <Button variant="outline" leftIcon={<Share2 className="w-5 h-5" />}>Share Data</Button>
            </div>
            ...
            <Card className="mt-16 bg-slate-50/50 dark:bg-slate-900/50 border-dashed border-2">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-sm font-black uppercase tracking-[0.2em] flex items-center gap-3">
                  <MessageSquare className="w-5 h-5 text-emerald-500" /> Community Discussion
                </h2>
                <span className="text-[10px] font-black uppercase text-slate-400 bg-white dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">0 Comments</span>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end">
                  <Button variant="primary" leftIcon={<Send className="w-4 h-4" />}>Post Comment</Button>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <Card className="!p-6 border-emerald-500/10">
              ...
              <p className="text-sm font-black uppercase tracking-tighter mb-4">{issue.address}</p>
            ...
            {/* REPORTER INFO */}
            <Card className="!p-6">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6">Citizen Reporter</h3>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-tighter text-slate-900 dark:text-white">{issue.user.name}</p>
                  <p className="text-[9px] font-black uppercase text-emerald-500 tracking-widest">Trust Rating: 98%</p>
                </div>
              </div>
            </Card>
            ...
            <Card className="!p-0 overflow-hidden bg-emerald-600 text-white border-none shadow-2xl shadow-emerald-600/30">
              <div className="p-8 space-y-8">
                <div className="flex items-center justify-between">
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-100">Official Response</h3>
                  <ShieldCheck className="w-6 h-6 text-emerald-300 opacity-50" />
                </div>
                ...
                <div className="space-y-8 relative">
                  ...
                  <div className="relative flex gap-6">
                    <div className="w-6 h-6 rounded-full bg-emerald-400 flex items-center justify-center ring-4 ring-emerald-600 z-10">
                      <CheckCircle2 className="w-3.5 h-3.5" />
                    </div>
                    <div className="flex-1">
                      <p className="text-xs font-black uppercase tracking-tight">Case Acknowledged</p>
                      <p className="text-[10px] text-emerald-100/70 font-bold">24 Dec, 09:12 AM</p>
                    </div>
                  </div>

                  <div className="relative flex gap-6">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ring-4 ring-emerald-600 z-10 ${issue.status === 'in-progress' ? 'bg-white' : 'bg-white/20'}`}>
                      <Clock className={`w-3.5 h-3.5 ${issue.status === 'in-progress' ? 'text-emerald-600' : 'text-white'}`} />
                    </div>
                    <div className={`flex-1 ${issue.status !== 'in-progress' && 'opacity-50'}`}>
                      <p className="text-xs font-black uppercase tracking-tight">Crew Assigned</p>
                      <p className="text-[10px] text-emerald-100/70 font-bold">24 Dec, 02:45 PM</p>
                    </div>
                  </div>
                  ...
                  <div className="relative flex gap-6 opacity-30">
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-black uppercase tracking-tight">Field Verification</p>
                      <p className="text-[10px] text-emerald-100/70 font-bold italic">Estimated: 26 Dec</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-emerald-700/50 p-6 text-center">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-200">Official Tracking ID: CP-0842-OAK</p>
              </div>
            </Card>
          </div>
        </div>
